...
All three applications are configured to run on different ports.
...
To run on specific ports and prevent cookie conflicts, the Customers and Products applications require the following properties:

[source,properties,indent=0]
.customers/src/main/resources/application.properties
----
include::example$/customers/src/main/resources/application.properties[tags=port]
----

[source,properties,indent=0]
.products/src/main/resources/application.properties
----
include::example$/products/src/main/resources/application.properties[tags=port]
----
...
Since Jmix 2.5, the generic REST API supports both named fetch plans defined in xref:data-access:fetching.adoc#creating-fetch-plans[fetch plans repository] and inline fetch plans defined in view descriptors or created programmatically. So the Orders application uses fetch plans for Customers and Products DTO entities in the same way as for local JPA entities. For example:

[source,xml,indent=0]
.orders/src/main/resources/com/company/orders/view/products/product/product-list-view.xml
----
include::example$/orders/src/main/resources/com/company/orders/view/products/product/product-list-view.xml[tags=fetch-plan]
----

Also, fetch plans can contain references to entities from a different data store:

[source,xml,indent=0]
.orders/src/main/resources/com/company/orders/view/orderline/order-line-detail-view.xml
----
include::example$/orders/src/main/resources/com/company/orders/view/orderline/order-line-detail-view.xml[tags=fetch-plan]
----
<1> Reference from OrderLine JPA entity to Product DTO entity.
...
[NOTE]
====
The `Product` instance received by the service method is not an entity loaded from the database. It contains only the attributes that were loaded in the Orders application, and it is in the new state. Use such a parameter only for reading, for example to obtain the product identifier, and don't save it. See xref:rest-ds:index.adoc#passing-entities-to-services[Passing Entities to Services] for details.
====

The Orders application invokes this service in the `OrderLine.detail` view to show the available quantity to the user. The service is represented by an interface annotated with `@RemoteService`:

[source,java,indent=0]
.orders/src/main/java/com/company/orders/service/InventoryService.java
----
include::example$/orders/src/main/java/com/company/orders/service/InventoryService.java[]
----

For interfaces marked with this annotation, the REST DataStore add-on generates client proxy objects that automatically serialize parameters, invoke the remote REST services, and deserialize the results.

The service interface can be used in the client code as a regular singleton bean:

[source,java,indent=0]
.orders/src/main/java/com/company/orders/view/orderline/OrderLineDetailView.java
----
include::example$orders/src/main/java/com/company/orders/view/orderline/OrderLineDetailView.java[tags=service]
----
